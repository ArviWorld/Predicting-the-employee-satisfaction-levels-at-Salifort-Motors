...
**Acknowledgement**\
I sincerely thank my parents and family for giving me the support and
...
heatmap = sns.heatmap(
    ...
    annot_kws={"size": 16},  # ← font size of annotation inside heatmap
...
ax[0].set_title('Tenure distribution classified by employee who left', fontsize=18)
...
ax[1].invert_yaxis()
ax[1].set_title('Satisfaction vs Tenure', fontsize=18)
...
ax[0].set_title('No of projects distribution classified by employee who left', fontsize=18)
...
ax[1].legend(loc='upper left', title='Left')
ax[1].invert_yaxis()
ax[1].set_title('Average monthly hours by No of project', fontsize=18)
plt.show()
```
...
ax[0].set_title('Satisfaction vs Salary', fontsize=18)
...
ax[0].set_title('Average monthly hours by promotion in the last 5 years', fontsize=18)

# Plot for avg_mon_hrs vs last_eval
sns.scatterplot(data=df1, x='avg_mon_hrs', y='last_eval', hue='left', alpha=0.4, ax=ax[1])
ax[1].set_title('Average monthly hours by evaluation score', fontsize=18)
...
plt.title('Employees distribution classified by department', fontsize=18)
...
predicting boolean outcomes.

Starting the logistic regression by setting the target and predictor
variables. Then training the model with the training dataset and then
using the test dataset to test the model.
...
powerful and intuitive methods used for both classification and
regression tasks.

**A Decision Tree** splits the data into subsets based on the value of
input features, creating a tree-like structure where each internal node
represents a decision rule. It is easy to interpret but can be prone to
overfitting.

To overcome this, **Random Forest**, an ensemble technique, builds
multiple Decision Trees on different random subsets of the data and
aggregates their predictions to improve accuracy and generalization.
Random Forests reduce variance and handle high-dimensional data well,
making them robust for complex modeling tasks.

Starting the tree based models by setting the target and predictor
variables. Then training the model with the training dataset and then
using the test dataset to test the model.
...
The Decision Tree model demonstrated strong performance, achieving high
scores across key metrics such as,
...
These results indicate that the model fits the data well. However, the
decision tree model is prone to overfitting. To address this concern and
ensure better generalization, `Random Forest` Model was performed to
compare the models.
...
The `Random Forest` model also demonstrated strong performance on the
training set, achieving high scores across key metrics such as,
...
These metrics indicate the model's strong ability to correctly classify
both classes while maintaining high generalization performance.

Based on the model training results,

-   `Random Forest` Model scores better than the Decision Tree, achieved
    higher precision, F1-score, accuracy, and AUC.
-   `Random Forest` helps reduce overfitting, a known issue with
    standalone Decision Trees and improving predictive reliability.
...
The `Random Forest` model also demonstrated strong performance on the
test set, achieving high scores across key metrics such as,
...
Relevant variables were selected and transformed to improve model
performance, including encoding categorical features, handling missing
values, and creating meaningful derived features to enhance predictive
accuracy. What can be engineered in this dataset,

-   Satisfaction level cannot be reported for all the employees. So,
    dropping it would be an option
-   Average monthly hours might have data leakage, as it might be
    recorded after the employees gives notice to resign or company has
    given the notice to leave. So, maybe engineering this variable to a
    new variable as overworked might help improve the models prediction
...
hours. **Overworked can be defined as working hours more than 175 hours
per month on average.** Therefore, employees working more than 175
hours/month were classified as overworked (1), while others were labeled
as not overworked (0).

To enrich the dataset with meaningful predictors, a new binary feature
overworked was engineered based on average monthly working hours. This
engineered feature adds interpretability to the model and allows it to
capture the potential impact of excessive working hours on employee
...
The `Decision tree 2` model balanced performance across all metrics, but
slightly underperforms compared to both `Decision Tree 1 CV` and
`Random Forest 1 CV`. such as,
...
The `Random Forest 2 CV` model balanced performance across all metrics,
but slightly underperforms compared to `Random Forest 1 CV` and slightly
better than `Decision Tree 2 CV`, with scores being,
...
-   So `Random Forest 1 CV` model is the winning model and the test can
    now be used for prediction
-   In general, `Random Forest` model performs well with ROC-AUC score
    as the deciding metric.

# Results and Discussion

This section presents the results of various machine learning models
applied to predict employee satisfaction at Salifort Motors. By
evaluating different classifiers—ranging from logistic regression to
ensemble methods—on cross-validation and test sets, we aim to identify
the most accurate and reliable model for understanding the key drivers
of employee satisfaction. The findings not only highlight model
performance but also offer actionable insights into workforce dynamics
and factors influencing satisfaction levels within the organization.
...
The model shows very low scores in the objective which is the importance
to predict employees who will leave. The data shows 83% - 17% split and
shows imbalance. The class distribution is imbalanced, with only 17% of
the data representing employees who left. This imbalance may contribute
to the model’s bias toward predicting that employees will stay.

Based on the insights and limitations, it is worth exploring alternative
classification models such as Decision Tree and Random Forest, which may
handle non-linear relationships and imbalanced data more effectively,
and potentially improve prediction of employee attrition.
...
`Decision Tree 1 (CV)` outperformed `Decision Tree 2 (CV)` across all
evaluation metrics. It achieved a higher precision, recall, and F1
score, indicating better balance between false positives and false
negatives. Additionally, it showed superior accuracy and AUC, suggesting
a more robust overall classification performance.
...
variable 'employee left'. In contrast, features like `department`,
`salary`, and `work accident` contribute minimally to the prediction.
This suggests that performance evaluation, workload, and time spent at
the company are key factors influencing employee attrition.
...
This decision tree uses several key features to classify employees as
satisfied (class: 0) or unsatisfied (class: 1). The most influential
variables include:

1.  **Number of Projects (#\_projects)\
    **If ≤ 2.5 projects:

    -   Most employees are satisfied, especially when:
        -   Their last evaluation is low (≤ 0.44),
        -   They are not overworked,
        -   Their tenure is short (≤ 2.5 years).
    -   A small group becomes unsatisfied if they have 3-year tenure and
        a slightly higher evaluation (\> 0.44).

    If \> 2.5 projects:

    -   Satisfaction largely depends on tenure, overwork status, and
        evaluation scores:
        -   Employees with many projects and longer tenure are usually
            satisfied, unless:
            -   They are overworked,
            -   Or have very high evaluation scores and specific project
                counts (e.g., \> 6.5 projects).

2.  **Last Evaluation (last_eval)**

    A strong factor throughout the tree:

    -   Low scores (≤ 0.57) often correlate with satisfaction,
        especially with fewer projects.
    -   Extremely high scores (\> 1.0) may indicate unsatisfied
        employees, particularly with low salary.

3.  **Overworked**

-   Overwork is consistently linked to employee dissatisfaction,
    especially among those with few projects and low evaluation scores.
-   Among overworked staff, those in Sales or Support departments, with
    mid-to-high evaluations, can show dissatisfaction.

4.  **Tenure**

-   Specific tenure groups (notably around 3 to 4 years) show higher
    risk of dissatisfaction, especially if paired with moderate
    evaluations and overwork.
-   Employees with short (\<3 years) or long (\>5 years) tenure are
    typically more satisfied.

5.  **Department and Salary Department plays a secondary role:**

-   Sales, IT, Support, and Technical departments appear frequently in
    branches where classification decisions are made.
-   Low salary, especially with very high evaluations, can lead to
    dissatisfaction.

**Key Patterns**

-   Employees with low workload, low evaluations, and short tenure are
    most likely to be satisfied.
-   Overworked employees, especially those with 3-4 years tenure and
    mid-high evaluations, are at higher risk of dissatisfaction.
-   Very high evaluation scores, low salary, and high project loads
    (\>6) can sometimes flag discontent.
...
no false negatives or false positives. The `Random Forest` models
demonstrated strong performance on the test set. These results highlight
the robustness and consistency of Random Forest classifiers in
predicting employee outcomes.

Comparing the two models, `Model 1 (baseline Random Forest)` outperforms
`Model 2 (feature-engineered Random Forest)` in terms of precision,
F1-score, accuracy, and AUC. While Model 2 shows a slight improvement in
recall, the drop in precision and overall performance
metrics suggests that the current feature engineering may have
introduced noise or redundant information rather than enhancing signal
quality.

In this case, the feature engineering did not improve model performance.
Instead, it slightly degraded the classifier's ability to distinguish
between classes. This highlights the importance of validating each
feature transformation step, as not all feature engineering enhances
model learning — it can also obscure useful signals or increase
dimensionality unnecessarily.

The `Random Forest 1` model consistently outperformed `Random Forest 2`
across both cross-validation (CV) and test datasets. In
cross-validation, `Random Forest 1` achieved a higher F1 score and
accuracy compared to `Random Forest 2`. Similarly, in the test set,
`Random Forest 1` yielded better performance with an F1 score and
accuracy than `Random Forest 2`. The higher AUC values across both CV
and test sets further confirm the superior classification performance of
`Random Forest 1`.The `Random Forest` model's performance will be similar when new unseen data is fitted, as the test data was used only for this model.
...
The `Random Forest` model (Test Set) clearly outperforms all other approaches in predicting employee satisfaction, achieving:

*Precision:* 96.42%
*F1-score:* 94.14%
*Accuracy:* 98.10%

This suggests that the model is not only accurate but also balanced in identifying both satisfied and unsatisfied employees with minimal false positives and false negatives.

**Insights**\
`Random Forest` is the most reliable model:

-   It handles feature interactions and non-linearities well, making it     a robust choice for complex, real-world data like employee              satisfaction.

Its high performance on the test set also indicates good generalization to unseen employee data.

**`Decision Tree (CV)` and `Random Forest (CV)` also show strong performance:**\
With F1-scores over 91% and AUC scores over 96%, these models are effective in cross-validated settings, suggesting that even simpler tree-based models are insightful.

**`Logistic Regression` is less effective:**\
While it's interpretable, its performance lags behind ensemble methods, likely due to its linear nature and limited capacity to model complex relationships.

**Model selection implications:**\
For business decision-making, `Random Forest` offers the best balance between precision and recall, helping HR departments accurately identify which employees are at risk of dissatisfaction. This allows for targeted interventions, retention strategies, and improved employee engagement.
...
with the model and feature importance. The modeling efforts successfully demonstrate that employee satisfaction at Salifort Motors can be predicted with high accuracy using advanced machine learning methods, particularly `Random Forest`. These insights can empower HR leaders to proactively enhance workplace satisfaction and reduce turnover.
